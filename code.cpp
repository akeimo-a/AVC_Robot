...
int main()
{
    ...
    while(1)
   {
       ...
       right_index = 0;
       right_sum = 0;
       IR_front = read_analog(0);
       //printf("%s\n","front");
       //printf("%d\n", IR_front);
       take_picture();
       //test = get_pixel(160, 120, 3);
       printf("%s","value");
       printf("%d", test);
		...
		top_value = get_pixel(160, 120, 3);
			...
			right_sum = right_sum + right_index;
       		...
       		if((top_index =1) && (bottom_index = 1) && (left_sum = 0) && (right_sum = 0)){
                ...
                set_motor(2, full_speed1); //go straight
                Sleep(0.5, 0);
       		}
			...
			set_motor(1, full_speed1);
			set_motor(2, full_speed1*2); //turn left
			Sleep(0.5, 0);
       		}
       		else if(right_sum > 0){
			set_motor(1, full_speed1*2);
			set_motor(2, full_speed1); //turn left
			Sleep(0.5, 0);
       		}
       } else if(IR_front > 600){
		IR_profile = read_analog(1);
		//printf("%s","profile");
		//printf("%d", IR_profile);
		if (IR_profile > 200){
			set_motor(1, stop);
